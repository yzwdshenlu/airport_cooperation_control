...
def evaluate(network, epoch, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        while not done and count < 501:
            ...
            a_in = [(action[0]) * 2 + 1, action[1]] # 动作空间为[-1,3] [-1,1]
            # 线速度[0,4] 角速度[-2,2]
...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(),lr=0.0003) # 用于优化 Actor 网络参数的优化器
        ...
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(),lr=0.0003) # 用于优化 Critic 网络参数的优化器
        ...
        self.visualizer = TrainingVisualizer() # 画图类
    ...
    def train(
        ...
        # 画图
        self.visualizer.update(self.iter_count,av_loss / iterations,av_Q / iterations,max_Q,iterations)
...
eval_freq = 2000  # After how many steps to perform the evaluation
max_ep = 100  # maximum number of steps per episode
eval_ep = 3  # number of episodes for evaluation
max_timesteps = 18000 # Maximum number of steps to perform
...
while timestep < max_timesteps:
    ...
    # a_in = [(action[0] + 1) * 2, action[1] * 2] # 动作空间为[0,4] [-2,2]
    a_in = [(action[0]) * 2 + 1, action[1]] # 动作空间为[-1,3] [-1,1]
...
if save_model:
    network.save("%s" % file_name, directory="./final_models")
np.save("./results/%s" % file_name, evaluations)
