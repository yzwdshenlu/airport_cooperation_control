...
class TrackingEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(500 + 2,), dtype=np.float32)
        self.current_point_index = 0
        self.robot_position = (0, 0)
        self.current_z = -1.57
        self.previous_z = -1.57
        self.reward = 0
        self.laser_data = np.zeros(500)
...
# 创建评估回调
env_callback = EvalCallback(env, best_model_save_path=log_dir, log_path=log_dir, eval_freq=10, deterministic=True, render=False)
...
model = TD3('MlpPolicy', env, verbose=1, learning_rate=0.03,action_noise=action_noise,train_freq=10,gradient_steps=1)
# 创建自定义进度条回调
progress_bar_callback = ProgressBarCallback(total_timesteps=10)
# 训练模型，传递回调函数
model.learn(total_timesteps=10, callback=[env_callback,progress_bar_callback])
# 保存模型
