...
class RobotInterface:
    ...
    def run(self):
        ...
        if (self.get_target):
            ...
            yaw = euler_angle.z # 偏航角
            cmd.linear.x = self.k_p_xy * (err_x * math.cos(yaw) + err_y * math.sin(yaw))
            if cmd.linear.x > self.limit_cmd_xy:
                cmd.linear.x = self.limit_cmd_xy
            if cmd.linear.x < -self.limit_cmd_xy:
                cmd.linear.x = -self.limit_cmd_xy
            cmd.linear.y = self.k_p_xy * (- err_x * math.sin(yaw) + err_y * math.cos(yaw))
            if cmd.linear.y > self.limit_cmd_xy:
                cmd.linear.y = self.limit_cmd_xy
            if cmd.linear.y < -self.limit_cmd_xy:
                cmd.linear.y = -self.limit_cmd_xy
...
if __name__ == "__main__":
    ...
    car_mecanum = RobotInterface('/robot1/airport_robot/odom', '/robot1/airport_robot/cmd_vel')
