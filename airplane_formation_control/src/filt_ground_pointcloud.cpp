...
#include <pcl/io/pcd_io.h>   
#include <pcl/filters/passthrough.h>
#include <pcl/filters/statistical_outlier_removal.h>  // ⭐️ 加入头文件
...
int main() {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered_outlier(new pcl::PointCloud<pcl::PointXYZ>);  // 增加
    ...
    std::cout << "After pass-through filter: " << cloud_filtered_z->points.size() << " points." << std::endl;

    // 统计离群点滤波器
    pcl::StatisticalOutlierRemoval<pcl::PointXYZ> sor;
    sor.setInputCloud(cloud_filtered_z);
    sor.setMeanK(50);                // 每个点考虑的邻居数
    sor.setStddevMulThresh(1.0);     // 离群判断的标准差倍数（越小越严格）
    sor.filter(*cloud_filtered_outlier);
    std::cout << "After outlier removal: " << cloud_filtered_outlier->points.size() << " points." << std::endl;

    // 保存最终处理后的点云
    pcl::io::savePCDFileASCII(output_file, *cloud_filtered_outlier);
    std::cout << "Saved filtered point cloud to " << output_file << std::endl;
