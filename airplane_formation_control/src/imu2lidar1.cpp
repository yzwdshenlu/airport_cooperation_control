...
/*****************************************************************************
 * 订阅激光雷达和IMU话题，将IMU数据变换成激光雷达的数据，并将变换后的数据发布，用于DLIO算法,仅适用于旋转激光雷达，需要有对应的joint_state！！！
 * 步骤：
 * 1.订阅话题
 * 2.读取激光雷达的旋转角速度和角度值
 * 3.将IMU数据变换为激光雷达数据
...
void imuCallback(const sensor_msgs::Imu::ConstPtr& msg)
{
  ...
  //定义IMU和激光雷达的位移向量
  Eigen::Vector3f IMU2Lidar_translation(0, 0, 0.2);//激光雷达在IMU坐标系下的位移
...
int main(int argc, char** argv) {
  ...
  ros::NodeHandle nh;
  ros::NodeHandle pnh("~");
  std::string robot_name = pnh.param<std::string>("robot_name", "default_robot");

  // 订阅关节状态消息
  ros::Subscriber sub1 = nh.subscribe(robot_name + "/airport_robot/joint_states", 10, jointStateCallback);

  // 订阅IMU消息
  ros::Subscriber sub2 = nh.subscribe(robot_name + "/imu", 1000, imuCallback);
  
  //订阅激光雷达旋转角速度
  ros::Subscriber sub3 = nh.subscribe(robot_name + "/airport_robot/vel", 1000, velCallback);

  imu_pub = nh.advertise<sensor_msgs::Imu>(robot_name + "/processed_imu", 10);
